...
\noindent The pitch elevation component now uses sex-adaptive parameters:
\begin{equation}
P = \min\left(1.0, \max\left(0, \frac{\bar{F_0} - B}{R}\right)\right) \quad \text{(pitch elevation)}
\end{equation}

\noindent where $(B, R)$ adapts based on estimated speaker sex:
\begin{equation}
(B, R) = \begin{cases}
    (120, 80) & \text{if } \bar{F_0}^{(\text{init})} < 165 \text{ Hz (estimated male)}\\
    (200, 100) & \text{otherwise (estimated female)}
\end{cases}
\end{equation}
...
\noindent The remaining components are:
\begin{align}
